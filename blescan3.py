...
def returnnumberpacket(pkt):
    ...
    multiple = 256
    for c in pkt:
        myInteger +=  bluetooth.get_byte(c) * multiple #struct.unpack("B",c)[0] * multiple #<---------- CHECK THIS IF NOT WORK
        multiple = 1
    return myInteger 
...
def returnstringpacket(pkt):
    myString = "";
    for c in pkt:
        myString +=  "%02x" % bluetooth.get_byte(c) #struct.unpack("B",c)[0] #<---------- CHECK THIS IF NOT WORK
    return myString 

def printpacket(pkt):
    for c in pkt:
        sys.stdout.write("%02x " % bluetooth.get_byte(c)) #struct.unpack("B",c)[0])
    print()
...
def parse_events(sock, loop_count=100):
    ...
    for i in range(0, loop_count):
        ...
        if event == bluez.EVT_INQUIRY_RESULT_WITH_RSSI:
            ...
            subevent = bluetooth.get_byte(pkt[3]) #struct.unpack("B", pkt[3])
            pkt = pkt[4:]
            if subevent == EVT_LE_CONN_COMPLETE:
                le_handle_connection_complete(pkt)
            elif subevent == EVT_LE_ADVERTISING_REPORT:
                #print("advertising report")
                num_reports = bluetooth.get_byte(pkt[0]) #struct.unpack("B", pkt[0])[0]
                report_pkt_offset = 0
                for i in range(0, num_reports):
        
                    if (DEBUG == True):
                        print("-------------")
                        print("\tfullpacket: ", pkt)
                        print("\tUDID: ")
                        ...
                        txpower = struct.unpack("b", bytes([pkt[report_pkt_offset -2]])) #struct.unpack("b", pkt[report_pkt_offset -2])
